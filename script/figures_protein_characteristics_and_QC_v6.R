...
df_aptamer <- annot %>%
  ...
  summarise(
    ...
    INTERVAL_LOD_batch1               = first(na.omit(INTERVAL_LOD_batch1)),
    INTERVAL_LOD_batch2               = first(na.omit(INTERVAL_LOD_batch2)),
    INTERVAL_Percent_under_LOD_batch1 = first(na.omit(INTERVAL_Percent_under_LOD_batch1)),
    INTERVAL_Percent_under_LOD_batch2 = first(na.omit(INTERVAL_Percent_under_LOD_batch2)),
    INTERVAL_CV_batch1                = first(na.omit(INTERVAL_CV_batch1)),
    INTERVAL_CV_batch2                = first(na.omit(INTERVAL_CV_batch2)),
    CHRIS_LOD                         = first(na.omit(CHRIS_LOD)),
    CHRIS_Percent_under_LOD           = first(na.omit(CHRIS_Percent_under_LOD)),
    CHRIS_CV                          = first(na.omit(CHRIS_CV)),
    .groups = "drop"
  ) %>%
  mutate(
    group_lbl = factor(
...
make_qc_violin <- function(long_df, ylab, y_limits = NULL) {
...
p1_e <- make_qc_violin(long_CV, ylab = "CV (%)", y_limits = c(0,50))
